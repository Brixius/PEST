class stair:
	...
	def adjust(self, wasCorrect):
		...
		if wasCorrect:
			if self.repeats == 0:
				self.repeats = 2
				#If this is the first trial (only time repeats will be 0), a
				#correct response sets repeats to 2.  As seen below, an
				#incorrect response sets repeats to -2.  This is because we
				#want to double the step size after three contiguous right or
				#wrong responses, and the script doubles the step size whenever
				#repeats is >=4 or <= -4.  Thus, 2, 3, 4, double.  -2, -3, -4,
				#double.  We do not use 1 or -1 here because, per rule 4 of
				#PEST, if a reversal (going from a string of right responses to
				#a wrong response, or vice versa) occurs immediately after a
				#step size doubling, one extra contiguous response must be made
				#to get another doubling (4, instead of 3).  When this
				#reversal-after-doubling occurs the repeats are set to 1 or -1.
				#When a reversal occurs NOT after a doubling, the repeats are
				#again set to 2 or -2.  This way, it normally takes 3
				#contiguous correct / incorrect responses to double step size,
				#but on the rare reversal-after-doubling event, it takes 4, per
				#PEST.
			else:
